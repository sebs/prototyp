...
for(var _i2=newChain.length-1;0<=_i2;_i2--){callbacks=this.__runOnBeforeLeaveCallbacks(callbacks,newContext,{prevent:prevent},previousChain[_i2])}// execute onBeforeEnter for changed segment element when skipping attach
for(var _i3=0;_i3<newChain.length;_i3++){callbacks=this.__runOnBeforeEnterCallbacks(callbacks,newContext,{prevent:prevent,redirect:redirect},newChain[_i3]);previousChain[_i3].element.location=createLocation(newContext,previousChain[_i3].route)}}else{// execute onBeforeLeave when NOT skipping attach
...
window.Vaadin=window.Vaadin||{};/**
                                    ...
                                    // Intentionally ignored as this is not a problem in the app being developed
                                     ...
                                     vaadin-dev-mode:end **/}var usageStatistics=function usageStatistics(){if("function"===typeof runIfDevelopmentMode){return runIfDevelopmentMode(maybeGatherAndSendStats)}};window.Vaadin=window.Vaadin||{};window.Vaadin.registrations=window.Vaadin.registrations||[];window.Vaadin.registrations.push({is:"@vaadin/router",version:"1.7.2"});usageStatistics();Router.NavigationTrigger={POPSTATE:POPSTATE,CLICK:CLICK};var vaadinRouter={Resolver:Resolver,Router:Router};_exports.$vaadinRouter=vaadinRouter;function init(outlet){var router=new Router(outlet);router.setRoutes([{path:"/",redirect:_vaadinApp.INHALTE},{path:_vaadinApp.QUELLEN,component:"sources-list",action:function action(){new Promise(function(res,rej){return _require.default(["../views/sources-list.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$sourcesList||{}})}},{path:_vaadinApp.INHALTE,component:"content-list",action:function action(){new Promise(function(res,rej){return _require.default(["../views/content-list.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$contentList||{}})}},{path:_vaadinApp.STATISTIKEN,component:"statistics-list",action:function action(){new Promise(function(res,rej){return _require.default(["../views/statistics-list.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$statisticsList||{}})}},{path:"(.*)",component:"app-404",action:function action(){new Promise(function(res,rej){return _require.default(["../views/404.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$404||{}})}}])}var appRouting={init:init};_exports.$appRouting=appRouting});