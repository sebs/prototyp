...
if(null==this._savedTransformAttr){this._savedTransformAttr=this._element.getAttribute("transform")}this._element.setAttribute("transform",scope.transformToSvgMatrix(value))}},_clear:function _clear(property){this._style[property]=this._surrogateStyle[property];if(this._updateSvgTransformAttr&&"transform"==scope.unprefixedPropertyName(property)){if(this._savedTransformAttr){this._element.setAttribute("transform",this._savedTransformAttr)}else{this._element.removeAttribute("transform")}this._savedTransformAttr=null}delete this._isAnimatedProperty[property]}};// Wrap the style methods.
for(var method in styleMethods){AnimatedCSSStyleDeclaration.prototype[method]=function(method,modifiesStyle){return function(){var result=this._surrogateStyle[method].apply(this._surrogateStyle,arguments);if(modifiesStyle){if(!this._isAnimatedProperty[arguments[0]])this._style[method].apply(this._style,arguments);this._updateIndices()}return result}}(method,method in styleMutatingMethods)}// Wrap the style.cssProperty getters and setters.
for(var property in document.documentElement.style){if(property in styleAttributes||property in styleMethods){continue}(function(property){configureProperty(AnimatedCSSStyleDeclaration.prototype,property,{get:function get(){return this._surrogateStyle[property]},set:function set(value){this._surrogateStyle[property]=value;this._updateIndices();if(!this._isAnimatedProperty[property])this._style[property]=value}})})(property)}function ensureStyleIsPatched(element){if(element._webAnimationsPatchedStyle)return;var animatedStyle=new AnimatedCSSStyleDeclaration(element);try{configureProperty(element,"style",{get:function get(){return animatedStyle}})}catch(_){// iOS and older versions of Safari (pre v7) do not support overriding an element's
// style object. Animations will clobber any inline styles as a result.
element.style._set=function(property,value){element.style[property]=value};element.style._clear=function(property){element.style[property]=""}}// We must keep a handle on the patched style to prevent it from getting GC'd.
